-- 시간대별 접속 분포
--
-- site_statistics 는 하루 한 행이라 "몇 시에 사람이 몰리는가"를 알 수 없었다.
-- site_visit_log 는 PK 가 (visit_date, ip_address) 라 하루 한 사람당 한 행만 남고
-- 시각도 없다. 그래서 별도 집계 테이블을 둔다.
--
-- 기존 두 테이블은 건드리지 않는다. site_visit_log 의 PK 를 바꾸면 하루 1행 제약이
...
CREATE TABLE IF NOT EXISTS site_visit_hourly (
